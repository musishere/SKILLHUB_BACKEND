...
export class TeamPlanController {
  // Create a team plan
  async createTeamPlan(req, res) {
    logger.info({ user: req.user?.id, body: req.body }, 'Create team plan request received');
    try {
      const userId = req.user?.id;
      const data = req.body;
      const teamPlan = await teamPlanService.createTeamPlan(userId, data);
      logger.info({ teamPlan }, 'Team plan created successfully');
      await publishToQueue('team_plan_events', { type: 'team_plan_created', userId, data });
      logger.info({ queue: 'team_plan_events', userId, data }, 'Team plan event pushed to RabbitMQ');
      res.send(teamPlan);
    } catch (err) {
